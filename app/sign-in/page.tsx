    ...
    <main className="min-h-screen bg-white font-sans text-stone-900 relative overflow-hidden">
      ...
      <header className="relative z-10 border-b border-stone-200 bg-white/80 backdrop-blur-xl">
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6 h-14 sm:h-16 flex items-center justify-between gap-3">
          ...
          <Link href="/waitlist" className="hidden sm:inline-flex text-[12px] font-semibold bg-black text-white px-5 py-2 rounded-full hover:bg-electric-blue transition-colors">{t("nav.bookCall")}</Link>
        </div>
      </header>
      ...
      <section className="relative z-10 max-w-[580px] mx-auto px-4 sm:px-6 py-12 sm:py-16">
        <div className="text-center mb-8 sm:mb-12">
          ...
          <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-electric-blue mb-4">{t("signIn.title")}</p>
          <h1 className="font-serif text-3xl sm:text-4xl tracking-tight font-normal mb-3">{t("signIn.subtitle")}</h1>
          <p className="text-stone-600 text-[14px] leading-relaxed font-medium">{t("signIn.description")}</p>
        </div>
        ...
        {sent ? (
          <div className="border border-stone-200 rounded-2xl p-6 sm:p-10 text-center bg-white/90 backdrop-blur-sm shadow-sm">
            ...
            <h2 className="font-serif text-2xl mb-3">{t("signIn.checkEmail")}</h2>
            <p className="text-stone-500 text-[14px] mb-2">{t("signIn.magicLinkSent")} <span className="font-semibold text-stone-800">{email}</span>.</p>
            <p className="text-stone-400 text-[12px]">{t("signIn.authNote")}</p>
            <Link href="/" className="inline-block mt-6 text-[13px] font-bold text-electric-blue underline underline-offset-4">{t("signIn.backHome")}</Link>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6 bg-white/90 backdrop-blur-sm border border-stone-200 rounded-2xl p-5 sm:p-8 shadow-sm">
            <div>
              <label className="text-[11px] font-bold tracking-wider uppercase text-stone-400 mb-1.5 block">{t("signIn.email")}</label>
              <input
                type="email"
                required
                value={email}
            ...
            <div className="rounded-xl border border-blue-100 bg-blue-50/60 p-4 flex gap-3">
              ...
              <div>
                <p className="text-[13px] font-semibold text-stone-800 mb-1">{t("signIn.magicLink")}</p>
                <p className="text-[12px] leading-relaxed text-stone-500">{t("signIn.noPassword")}</p>
              </div>
            </div>
            ...
            >
              {loading ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> {t("signIn.sending")}</> : <>{t("signIn.continue")} <ArrowRight size={15} /></>}
            </button>

            <p className="text-[11px] text-center text-stone-400">
              {t("signIn.noAccount")} <Link href="/waitlist" className="text-electric-blue font-bold">{t("signIn.startFreeTrial")}</Link>
            </p>
          </form>
        )}
      </section>
